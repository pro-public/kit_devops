...
DIR1="/tmp/estructura_server1_$(date +%Y%m%d%H%M%S).txt"
DIR2="/tmp/estructura_server2_$(date +%Y%m%d%H%M%S).txt"
LOG_FILE="comparacion_estructura_$(date +%Y%m%d%H%M%S).log"

# Obtener la estructura de los directorios en ambos servidores
echo "Obteniendo estructura de directorios desde $SERVER1..."
ssh $SERVER1 "find $REMOTE_DIR -type f | sort" > $DIR1

echo "Obteniendo estructura de directorios desde $SERVER2..."
ssh $SERVER2 "find $REMOTE_DIR -type f | sort" > $DIR2

# Comparar las estructuras de los directorios
echo "Comparando estructuras de los directorios..."

echo "Archivos que están en $SERVER1 pero no en $SERVER2:" | tee -a $LOG_FILE
comm -23 $DIR1 $DIR2 | tee -a $LOG_FILE

echo "" | tee -a $LOG_FILE
echo "Archivos que están en $SERVER2 pero no en $SERVER1:" | tee -a $LOG_FILE
